...
export const fetchProjects = async (
  ...
  try {
    ...
    const response = await fetch(
      `${API_URL}/AllProjects?pageSize=${pageSize}&pageNum=${pageNum}${selectedCategories.length ? `&${categoryParams}` : ''}`, // Back ticks allow you to use variables in the string, so you can change the page size
    );
...
export const addProject = async (newProject: Project): Promise<Project> => {
  try {
    ...
    console.log('Add project request completed');
  }
};
